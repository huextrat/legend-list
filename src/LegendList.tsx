...
export const LegendList = React.forwardRef(
  <T,>(props: LegendListProps<T>, forwardedRef: ForwardedRef<ScrollView>) => {
    const {
      data,
      initialScrollIndex,
      initialScrollOffset,
    ...
    const refScroller = (forwardedRef ||
      internalRef) as React.MutableRefObject<ScrollView>;
    ...
    if (!refPositions.current) {
      ...
      refPositions.current.idsInFirstRender = new Set(
        data.map((_: T, i: number) => getId(i)),
      );
    }
    refPositions.current.data = data;

    const SCREEN_LENGTH =
      Dimensions.get("window")[horizontal ? "width" : "height"];

    const initialContentOffset =
      initialScrollOffset ??
      (initialScrollIndex
        ? initialScrollIndex * estimatedItemLength(initialScrollIndex)
        : undefined);
    ...
    const allocateContainers = useCallback(() => {
      const numContainers =
        initialContainers ||
        Math.ceil((SCREEN_LENGTH + scrollBuffer * 2) / estimatedItemLength(0)) +
          4;
    ...
    const calculateItemsInView = useCallback(() => {
      ...
      const prevRange = onViewableRangeChanged
        ? { ...visibleRange$.peek() }
        : undefined;
        ...
        for (let i = startBuffered; i <= endBuffered; i++) {
          ...
          if (!isContained) {
            ...
            for (let u = 0; u < containers.length; u++) {
              ...
              if (
                container.itemIndex < startBuffered ||
                container.itemIndex > endBuffered
              ) {
    ...
    const updateItemLength = useCallback((index: number, length: number) => {
      ...
      const prevLength =
        lengths.get(id) || (wasInFirstRender ? estimatedItemLength(index) : 0);
      ...
      if (!prevLength || prevLength !== length) {
        ...
        visibleRange$.totalLength.set(
          (prevTotal) => prevTotal + (length - prevLength),
        );
    ...
    const handleScrollDebounced = useCallback(() => {
      ...
      const distanceFromEnd =
        visibleRange$.totalLength.peek() - newScroll - SCREEN_LENGTH;
      if (refPositions.current) {
        refPositions.current.isAtBottom =
          distanceFromEnd < SCREEN_LENGTH * autoScrollToBottomThreshold;
      }
    ...
    const handleScroll = useCallback(
      (
        ...
        if (refPositions.current && !refPositions.current.animFrame) {
          refPositions.current.animFrame = requestAnimationFrame(
    ...
    useEffect(() => {
      if (initialContentOffset) {
        handleScroll({
          nativeEvent: { contentOffset: { x: 0, y: initialContentOffset } },
        });
        calculateItemsInView();
      }
    }, []);
    ...
    return (
      ...
      >
        {ListHeaderComponent && (
          <Reactive.View $style={ListHeaderComponentStyle}>
            {ListHeaderComponent}
          </Reactive.View>
        )}
        ...
        {ListFooterComponent && (
          <Reactive.View $style={ListFooterComponentStyle}>
            {ListFooterComponent}
          </Reactive.View>
        )}
      </Reactive.ScrollView>
